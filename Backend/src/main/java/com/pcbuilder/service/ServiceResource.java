...
import com.pcbuilder.dao.CategoryDao;
import com.pcbuilder.dao.ServiceDao;
import com.pcbuilder.dao.ServiceNegotiationDao;
import com.pcbuilder.dao.ServiceRequestDao;
import com.pcbuilder.dao.ServiceRequestMessageDao;
import com.pcbuilder.dao.UserDao;
import com.pcbuilder.dto.AddNegotiationRequest;
import com.pcbuilder.dto.AddServiceRequest;
import com.pcbuilder.dto.CommonApiResponse;
import com.pcbuilder.dto.CustomerAddServiceRequest;
import com.pcbuilder.dto.ServiceRequestResponse;
import com.pcbuilder.dto.ServiceResponse;
...
import com.pcbuilder.exception.ServiceSaveFailedException;
import com.pcbuilder.utility.Constants.ActiveStatus;
import com.pcbuilder.utility.Constants.ServiceCategory;
import com.pcbuilder.utility.Constants.ServiceNegotiationStatus;
import com.pcbuilder.utility.Constants.ServiceRequestStatus;
...
public class ServiceResource {
	...
	public ResponseEntity<CommonApiResponse> addService(AddServiceRequest request) {
		...
		if (request.getName() == null || request.getDescription() == null || request.getCategory() == null
				|| request.getMinPrice() == null || request.getDeliveryTime() == 0 || request.getTechExpertId() == 0
				|| request.getImage1() == null || request.getImage2() == null || request.getImage3() == null) {
			response.setResponseMessage("Fields are empty");
			response.setSuccess(false);
		...
		if (request.getCategory().equals(ServiceCategory.PC_BUILDER.value())) {
			subCategory = this.categoryDao.findById(request.getSubCategoryId()).get();

			if (subCategory == null) {
				response.setResponseMessage("Sub Category not found");
				response.setSuccess(false);
	...
	public ResponseEntity<ServiceResponse> fetchAllServices() {
		...
		response.setResponseMessage("Services fetched successfully");
		response.setSuccess(true);
	...
	public ResponseEntity<ServiceResponse> fetchAllServicesByCategory(String category) {

		LOG.info("Request received for fetching all services using category");
	...
	public ResponseEntity<ServiceResponse> fetchAllServicesBySubCategory(Integer subCategoryId) {
		...
		if (subCategoryId == 0) {
			response.setResponseMessage("Missing sub category");
			response.setSuccess(false);
	...
	public ResponseEntity<CommonApiResponse> addServiceRequestNegotiationCustomer(AddNegotiationRequest request) {
		...
		if (request.getStatus().equals(ServiceNegotiationStatus.APPROVED.value())) {
			...
			if(customer.getWalletAmount().compareTo(negotiation.getPrice()) < 0) {
				response.setResponseMessage("Insufficient fund in your wallet, Can't approve the Tech Expert Plan now.");
				response.setSuccess(false);
